...
function Toast({msg,onDone}){
  ...
  return <div onClick={onDone} style={{position:"fixed",bottom:24,left:"50%",transform:"translateX(-50%)",zIndex:9999,background:C.green,color:"#fff",fontFamily:"'Heebo',sans-serif",fontWeight:700,fontSize:14,padding:"13px 26px",borderRadius:12,cursor:"pointer",boxShadow:`0 6px 24px ${C.green}55`,animation:"slideUp .3s ease",maxWidth:"90vw",lineHeight:1.5,direction:"rtl",textAlign:"center"}}>{msg}</div>;
}
function Spinner(){
  return <div style={{display:"flex",alignItems:"center",justifyContent:"center",height:"100vh",flexDirection:"column",gap:16,fontFamily:"'Heebo',sans-serif",color:C.muted}}>
    ...
    <span style={{fontSize:14}}>טוען נתונים...</span>
  </div>;
}

const iStyle=(extra={})=>({background:C.bgSoft,border:`1.5px solid ${C.border}`,borderRadius:8,color:C.text,padding:"10px 14px",width:"100%",fontFamily:"'Heebo',sans-serif",fontSize:14,direction:"rtl",outline:"none",...extra});
const btn=(bg=C.green,col="#fff",extra={})=>({background:bg,border:"none",borderRadius:8,cursor:"pointer",color:col,fontFamily:"'Heebo',sans-serif",fontWeight:700,fontSize:13,padding:"9px 14px",transition:"opacity .15s",...extra});

function VisitPopup({visit,emp,onClose,onCancel}){
  return(
    <div style={{position:"fixed",inset:0,background:"#00000060",zIndex:3000,display:"flex",alignItems:"center",justifyContent:"center"}} onClick={onClose}>
      <div onClick={e=>e.stopPropagation()} dir="rtl" style={{background:C.bg,borderRadius:18,padding:24,width:350,maxWidth:"92vw",boxShadow:"0 20px 60px #0004",fontFamily:"'Heebo',sans-serif",animation:"slideUp .25s ease",maxHeight:"90vh",overflowY:"auto"}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:8}}>
          {[
            ...
            ["🕐 שעת התחלה",visit.hour],
            ["🕕 שעת סיום",addH(visit.hour,visit.duration)],
            ...
            <div key={label} style={{display:"flex",gap:10,alignItems:"flex-start",padding:"8px 10px",background:C.bgSoft,borderRadius:8}}>
              <span style={{fontSize:12,color:C.muted,minWidth:96,flexShrink:0}}>{label}</span>
              <span style={{fontSize:13,fontWeight:600,color:C.text,wordBreak:"break-word"}}>{val}</span>
            </div>
          ))}
        </div>
        <button onClick={()=>{onCancel(visit.id);onClose();}} style={{marginTop:18,width:"100%",background:"#FEE2E2",border:"none",borderRadius:9,color:"#DC2626",fontFamily:"'Heebo',sans-serif",fontWeight:700,fontSize:14,padding:"11px",cursor:"pointer"}}>🗑 בטל ביקור</button>
      </div>
    </div>
  );
}
...
function BookingModal({slot,employees,isAvailable,isBooked,onConfirm,onClose}){
  ...
  return(
    <div style={{position:"fixed",inset:0,background:"#00000060",zIndex:2000,display:"flex",alignItems:"center",justifyContent:"center"}} onClick={onClose}>
      <div onClick={e=>e.stopPropagation()} dir="rtl" style={{background:C.bg,border:`1.5px solid ${C.border}`,borderRadius:18,padding:22,width:390,maxWidth:"95vw",boxShadow:"0 20px 60px #0003",fontFamily:"'Heebo',sans-serif",animation:"slideUp .25s ease",maxHeight:"92vh",overflowY:"auto"}}>
        <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:16}}>
          <Avatar emp={emp} size={42}/>
          <div style={{flex:1}}><div style={{fontSize:16,fontWeight:800,color:C.text}}>קביעת ביקור</div><div style={{fontSize:12,color:C.muted}}>{emp.name} · {getDayHe(slot.date)} {fmtDate(slot.date)} · {slot.hour}</div></div>
        ...
        <div style={{display:"flex",gap:10,marginTop:16}}>
          <button onClick={handleConfirm} style={{flex:1,background:C.green,border:"none",borderRadius:9,color:"#fff",fontFamily:"'Heebo',sans-serif",fontWeight:700,fontSize:15,padding:"12px",cursor:"pointer"}}>✓ אשר ביקור</button>
          <button onClick={onClose} style={{background:C.light,border:"none",borderRadius:9,color:C.navy,fontFamily:"'Heebo',sans-serif",fontWeight:700,fontSize:14,padding:"12px 16px",cursor:"pointer"}}>ביטול</button>
        </div>
      </div>
    </div>
  );
...
function EmpModal({emp,onSave,onDelete,onClose}){
  ...
  if(confirmDelete){
    return(
      <div style={{position:"fixed",inset:0,background:"#00000060",zIndex:4000,display:"flex",alignItems:"center",justifyContent:"center"}} onClick={()=>setConfirmDelete(false)}>
        <div onClick={e=>e.stopPropagation()} dir="rtl" style={{background:C.bg,border:`1.5px solid ${C.border}`,borderRadius:18,padding:28,width:320,maxWidth:"88vw",boxShadow:"0 20px 60px #0003",fontFamily:hb,animation:"slideUp .2s ease",textAlign:"center"}}>
  ...
  return(
    <div style={{position:"fixed",inset:0,background:"#00000060",zIndex:4000,display:"flex",alignItems:"center",justifyContent:"center"}} onClick={onClose}>
      <div onClick={e=>e.stopPropagation()} dir="rtl" style={{background:C.bg,border:`1.5px solid ${C.border}`,borderRadius:18,padding:24,width:340,maxWidth:"92vw",boxShadow:"0 20px 60px #0003",fontFamily:hb,animation:"slideUp .25s ease"}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:14}}>
          ...
          <div>
            ...
            <div style={{display:"flex",gap:9,flexWrap:"wrap"}}>
              {EMP_COLORS.map(c=>(<button key={c} onClick={()=>setForm(p=>({...p,color:c}))} style={{width:34,height:34,borderRadius:"50%",background:c,border:form.color===c?`3px solid ${C.text}`:"3px solid transparent",cursor:"pointer"}}/>))}
            </div>
          </div>
        </div>
...
export default function MapitAgenda(){
  ...
  function getBookedVisit(id,d,h){
  ...
  async function toggleSlot(empId,date,hour){
    ...
    const ref=doc(db,"availability",empId);
    const current=availability[empId]||{};
    await setDoc(ref,{...current,[date]:newSlots});
  }
  ...
  async function deleteEmployee(id){
    ...
    const vToDelete=visits.filter(v=>v.empId===id);
    for(const v of vToDelete)await deleteDoc(doc(db,"visits",v.id));
    await deleteDoc(doc(db,"availability",id));
    setToast("🗑️ עובד הוסר.");
    setEmpModal(null);
  ...
  function renderCell(empId,date,hour,onClick){
    const avail=isAvailable(empId,date,hour);
    const booked=isBooked(empId,date,hour);
    const past=date<todayStr();
    const isStart=isVisitStart(empId,date,hour);
    const emp=employees.find(e=>e.id===empId);
    return(
      <button onClick={()=>onClick(empId,date,hour)}
        title={booked?"לחץ לפרטי הביקור":avail&&!past?`קבע ביקור עם ${emp?.name}`:""}
        style={{width:"100%",height:30,borderRadius:6,border:"none",cursor:booked?"pointer":avail&&!past?"pointer":"default",background:booked?"#FFF3E0":avail?emp?.color+"22":C.bgSoft,borderWidth:1.5,borderStyle:"solid",borderColor:booked?C.orange:avail?emp?.color+"99":C.border,opacity:past&&!booked?0.3:1,fontSize:12,color:booked?C.orange:avail?emp?.color:"transparent",fontWeight:700,transition:"all .12s"}}>
  ...
  const CSS=`
    ...
    *{box-sizing:border-box;margin:0;padding:0;}
    ...
    .emprow:hover{background:${C.bgSoft}!important;cursor:pointer}
  ...
  if(view==="employee"&&selectedEmp){
    ...
    return(
      <div dir="rtl" style={{minHeight:"100vh",background:C.bg,color:C.text,fontFamily:"'Heebo',sans-serif"}}>
        <style>{CSS}</style>
        <div style={{background:C.bg,borderBottom:`1.5px solid ${C.border}`,padding:"14px 18px",display:"flex",alignItems:"center",justifyContent:"space-between",boxShadow:"0 2px 8px #0001"}}>
          <div style={{display:"flex",alignItems:"center",gap:12}}>
            <Avatar emp={emp} size={44}/>
            <div><div style={{fontSize:17,fontWeight:800}}>{emp.name}</div><div style={{fontSize:11,color:C.muted}}>✓ סמן זמינות · 📍 לחץ לפרטי ביקור</div></div>
          </div>
          <button onClick={()=>setView("admin")} style={btn(C.light,C.navy)}>→ חזור</button>
        </div>
        <div style={{padding:"9px 18px",display:"flex",gap:16,fontSize:11,color:C.muted,background:C.bgSoft,borderBottom:`1px solid ${C.border}`}}>
          {[[emp.color+"18",emp.color,"זמין"],[C.light,C.border,"לא זמין"],["#FFF3E0",C.orange,"ביקור"]].map(([bg,bc,l])=>(
            <span key={l} style={{display:"flex",alignItems:"center",gap:5}}><span style={{width:12,height:12,borderRadius:3,background:bg,border:`1.5px solid ${bc}`,display:"inline-block"}}/>{l}</span>
          ))}
        </div>
        <div style={{padding:"10px 18px",display:"flex",alignItems:"center",gap:10,borderBottom:`1px solid ${C.border}`,background:C.bg}}>
          <button onClick={()=>shiftWeek(-1,setEmpWeekBase,empWeekBase)} style={btn(C.light,C.navy)}>‹ שבוע קודם</button>
          <span style={{fontSize:13,color:C.navy,fontWeight:700,flex:1,textAlign:"center"}}>{fmtDate(empWeekDates[0])} – {fmtDate(empWeekDates[empWeekDates.length-1])}</span>
          <button onClick={()=>shiftWeek(1,setEmpWeekBase,empWeekBase)} style={btn(C.light,C.navy)}>שבוע הבא ›</button>
        </div>
        <div style={{overflowX:"auto",padding:"12px 16px 40px"}}>
          <table style={{borderCollapse:"separate",borderSpacing:"4px 4px",minWidth:500}}>
            <thead>
              <tr>
                <th style={{width:52,color:C.muted,fontSize:10,textAlign:"right",paddingRight:5}}>שעה</th>
                {empWeekDates.map(date=>(<th key={date} style={{color:date===todayStr()?emp.color:C.navy,fontSize:11,fontWeight:700,padding:"5px 3px",textAlign:"center",minWidth:72,borderBottom:date===todayStr()?`2.5px solid ${emp.color}`:`1.5px solid ${C.border}`}}><div>{getDayHe(date)}</div><div style={{fontSize:9,fontWeight:400,color:C.muted}}>{fmtDate(date)}</div></th>))}
              </tr>
            </thead>
            <tbody>
              {HOURS.map(hour=>(<tr key={hour}><td style={{color:C.muted,fontSize:10,textAlign:"right",paddingRight:5,verticalAlign:"middle"}}>{hour}</td>{empWeekDates.map(date=>(<td key={date}>{renderCell(emp.id,date,hour,handleEmpCell)}</td>))}</tr>))}
            </tbody>
          </table>
        </div>
  ...
  return(
    <div dir="rtl" style={{minHeight:"100vh",background:C.bgSoft,color:C.text,fontFamily:"'Heebo',sans-serif"}}>
      <style>{CSS}</style>
      <div style={{background:C.bg,borderBottom:`1.5px solid ${C.border}`,padding:"10px 22px",display:"flex",alignItems:"center",justifyContent:"space-between",boxShadow:"0 2px 10px #0001"}}>
        <div style={{display:"flex",alignItems:"center",gap:12}}>
          <img src={LOGO_SRC} alt="Mapit" style={{height:36,width:"auto",display:"block"}}/>
          <div style={{width:1.5,height:28,background:C.border,margin:"0 4px"}}/>
          <div><div style={{fontSize:13,fontWeight:800,color:C.navy}}>Agenda</div><div style={{fontSize:10,color:C.muted}}>מערכת ניהול ביקורים</div></div>
        </div>
        <div style={{fontSize:12,color:C.muted,fontWeight:600}}>{getDayHe(todayStr())} {fmtDate(todayStr())}</div>
      </div>
      <div style={{display:"flex",height:"calc(100vh - 65px)",overflow:"hidden"}}>
        <div style={{width:230,minWidth:180,background:C.bg,borderLeft:`1.5px solid ${C.border}`,display:"flex",flexDirection:"column",overflow:"hidden",flexShrink:0}}>
          ...
          <div style={{flex:1,overflowY:"auto",padding:"8px 10px"}}>
            {upcomingVisits.length===0&&<div style={{color:C.muted,fontSize:12,textAlign:"center",marginTop:36,opacity:.6}}>אין ביקורים</div>}
            {upcomingVisits.map(v=>{
              const emp=employees.find(e=>e.id===v.empId);
              return(<div key={v.id} onClick={()=>setViewVisit(v)} style={{background:C.bgSoft,borderRadius:10,padding:"10px 11px",marginBottom:8,borderRight:`3px solid ${emp?.color||C.border}`,cursor:"pointer",boxShadow:"0 1px 5px #0001",animation:"fadeIn .3s"}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:3}}>
                  <div style={{fontSize:13,fontWeight:700,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis",maxWidth:140}}>{v.contactName}</div>
                  <button onClick={e=>{e.stopPropagation();cancelVisit(v.id);}} style={{background:"transparent",border:"none",cursor:"pointer",color:C.muted,fontSize:15,lineHeight:1,padding:2,flexShrink:0}}>✕</button>
                </div>
                <div style={{fontSize:11,color:C.muted}}><span style={{color:emp?.color,fontWeight:700}}>{emp?.name}</span>{" · "}{fmtDate(v.date)}{" · "}{v.hour}–{addH(v.hour,v.duration)}</div>
                {v.city&&<div style={{fontSize:11,color:C.muted,marginTop:2}}>{v.city}{v.address?`, ${v.address}`:""}</div>}
                <div style={{marginTop:5}}><span style={{background:"#EFF8F3",borderRadius:4,padding:"2px 7px",fontSize:10,color:C.green,fontWeight:600}}>{v.service}</span></div>
              </div>);
            })}
          </div>
        </div>
        <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",minWidth:0}}>
          <div style={{padding:"10px 14px",borderBottom:`1px solid ${C.border}`,background:C.bg,display:"flex",alignItems:"center",gap:10,flexShrink:0}}>
            <button onClick={()=>shiftWeek(-1,setWeekBase,weekBase)} style={btn(C.light,C.navy)}>‹ שבוע קודם</button>
            ...
            <button onClick={()=>shiftWeek(1,setWeekBase,weekBase)} style={btn(C.light,C.navy)}>שבוע הבא ›</button>
          </div>
          <div style={{padding:"7px 14px",background:C.bgSoft,borderBottom:`1px solid ${C.border}`,display:"flex",gap:12,flexWrap:"wrap",flexShrink:0}}>
            {employees.map(emp=>(<span key={emp.id} style={{display:"flex",alignItems:"center",gap:5,fontSize:11,color:C.muted}}><span style={{width:8,height:8,borderRadius:2,background:emp.color,display:"inline-block"}}/>{emp.name}</span>))}
            <span style={{display:"flex",alignItems:"center",gap:5,fontSize:11,color:C.muted}}><span style={{width:8,height:8,borderRadius:2,background:"#FFF3E0",border:`1px solid ${C.orange}`,display:"inline-block"}}/>ביקור</span>
          </div>
          <div style={{flex:1,overflowY:"auto",overflowX:"auto",padding:"8px 10px 20px"}}>
            <table style={{borderCollapse:"separate",borderSpacing:"3px 3px",width:"100%"}}>
              <thead>
                <tr>
                  <th style={{width:46,color:C.muted,fontSize:10,textAlign:"right",paddingRight:4,paddingBottom:3}}>שעה</th>
                  {weekDates.map(date=>(<th key={date} colSpan={employees.length} style={{textAlign:"center",fontSize:10,fontWeight:700,padding:"4px 2px 6px",color:date===todayStr()?C.orange:C.navy,borderBottom:date===todayStr()?`2.5px solid ${C.orange}`:`1.5px solid ${C.border}`}}>{getDayHe(date)}<br/><span style={{fontWeight:400,color:C.muted,fontSize:9}}>{fmtDate(date)}</span></th>))}
                </tr>
                <tr><th/>{weekDates.map(date=>employees.map(emp=>(<th key={emp.id+date} style={{textAlign:"center",padding:"2px 1px 4px"}}><Avatar emp={emp} size={16}/></th>)))}</tr>
              </thead>
              <tbody>
                {HOURS.map(hour=>(<tr key={hour}><td style={{color:C.muted,fontSize:10,textAlign:"right",paddingRight:4,verticalAlign:"middle"}}>{hour}</td>{weekDates.map(date=>employees.map(emp=>(<td key={emp.id+date+hour} style={{padding:"1px"}}>{renderCell(emp.id,date,hour,handleAdminCell)}</td>)))}</tr>))}
              </tbody>
            </table>
          </div>
        </div>
        <div style={{width:210,minWidth:170,background:C.bg,borderRight:`1.5px solid ${C.border}`,display:"flex",flexDirection:"column",overflow:"hidden",flexShrink:0}}>
          <div style={{padding:"12px 12px 8px",borderBottom:`1px solid ${C.border}`,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <div style={{fontSize:10,fontWeight:700,color:C.muted,letterSpacing:1,textTransform:"uppercase"}}>עובדים</div>
            <button onClick={()=>setEmpModal({id:null,name:"",color:C.navy,initials:""})} style={btn(C.green,undefined,{fontSize:11,padding:"5px 10px"})}>+ חדש</button>
          </div>
          <div style={{flex:1,overflowY:"auto",padding:"8px 10px"}}>
            {employees.map(emp=>(
              <div key={emp.id} className="emprow" style={{display:"flex",alignItems:"center",gap:8,padding:"8px 8px",borderRadius:10,marginBottom:5,transition:"background .15s"}}>
                <div onClick={()=>{setSelectedEmp(emp);setView("employee");}} style={{display:"flex",alignItems:"center",gap:8,flex:1,minWidth:0,cursor:"pointer"}}>
                  <Avatar emp={emp} size={32}/>
                  <div style={{minWidth:0}}><div style={{fontSize:12,fontWeight:700,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{emp.name}</div><div style={{fontSize:10,color:C.muted}}>{Object.values(availability[emp.id]||{}).flat().length} שע' זמינות</div></div>
                </div>
                <button onClick={()=>setEmpModal(emp)} style={{background:"transparent",border:"none",cursor:"pointer",color:C.muted,fontSize:15,padding:3,flexShrink:0}}>✏️</button>
              </div>
            ))}
          </div>
        </div>
